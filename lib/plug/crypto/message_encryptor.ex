defmodule Plug.Crypto.MessageEncryptor do
  ...
  def encrypt_and_sign(message, secret, sign_secret, cipher \\ nil)
    ...
    IO.puts(
      :stderr,
      "warning: `Plug.Crypto.MessageEncryptor.encrypt_and_sign/4` is deprecated," <>
  ...
  def verify_and_decrypt(encrypted, secret, sign_secret, cipher \\ nil)
    ...
    IO.puts(
      :stderr,
      "warning: `Plug.Crypto.MessageEncryptor.verify_and_decrypt/4` is deprecated," <>
  ...
  defp decode_legacy_token(token, sign_secret) do
    ...
    with [cipher_text, cipher_tag] when byte_size(cipher_text) > 0 and byte_size(cipher_tag) > 0 <-
           split,
