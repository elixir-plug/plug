defmodule Plug.Adapters.Cowboy2.ConnTest do
  ...
  test "https" do
    ...
    assert {:ok, 200, _headers, client} = :hackney.get("https://127.0.0.1:8004/https", [], "", ssl_options)
  ...
  defp request(:head = verb, path) do
    {:ok, status, headers} =
      :hackney.request(verb, "http://127.0.0.1:8003" <> path, [], "", [])
    {status, headers, nil}
  end
